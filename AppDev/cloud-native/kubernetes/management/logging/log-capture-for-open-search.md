...
# Cloud Native - Log capture for Open Search
...
This module shows how to create an OPen Search instance (this is a OCI managed service that is a compatible with Elastic) install and configure the log capture tool Fluentd, and write data to the Open Search instance where it could be subsequently processed or analyzed (the actual processing / analysis is not covered in this module)

## Task 1: Creating the Open Search instance
...
To process log data in a consistent manner we need to get all of the data into one place. We're going to use [fluentd](https://www.fluentd.org/) to capture the data and send it to  an Elastic search instance deployed in our Kubernetes cluster in the initial example below, but there are many other options.
  ...
  ![](images/opensearch-service-access.png)

  1. Navigate to the Open Search cluster page, click the "Hamburger" manu, then select **Databases**, and on the database service page in the **Open Search** section select **Clusters**
  
  ![](images/opensearch-cluster-start-workflow.pn)
  
  2. Click the **Create** button to start the cluster creation
  
  ![](images/opensearch-cluster-create-stage1-type.png)
  
  3. Name the cluster `logsearch` make sure that the `CTDOKE` compartment is selected. Click the **Next** button
  
  ![](images/opensearch-cluster-create-stage2-nodes.png)
  
  4. Make sure that the node optimization is set to **Developer** - this will apply the settings to create a small cluster to explore. You can chose a larger cluster shape if you want, but that's going to be overkill for this lab. Click the **Next** button.
   
  ![](images/openseaarch-cluster-create-stage3-networking.png)
   
  5. Make sure the **Select existing virtual cloud network** option is chosen and select the compartment, this will be something like `lab-one-oke-vcn-tg` and will end with your initials. Make sure that the **Select existing subnet** option is chosen and chose the load balancer subnet, this will be names something like `lab-one-pub_lb`. Click the **Next** button.
  
  ![](images/opensearch-cluster-create-stage4-summary.png)
  
  6. On the summary page you can confirm the details you have entered. Click the **Create cluster** button. 
